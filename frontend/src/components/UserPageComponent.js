...
import { CardHeader, Container, FormControl, InputLabel, LinearProgress, ListItemText } from "@material-ui/core";
...
class userPageComponent extends React.Component {
    ...
    render() {
        ...
        debugger
        const admin = window.localStorage;
        return (
                    ...
                    {user ? <Card>
                        ...
                        <CardContent>
                            <Grid container
                                ...
                                <h4>Ролі:</h4>
                                {JSON.parse(window.localStorage.getItem('user')).roles[0] === 'ROLE_TEACHER' ? (



                                    <FormControl sx={{width: '300'}} style={{minWidth: 200}}>

                                        <InputLabel id="roles_label">{!this.state.edit ? 'Роль': editUser.roles[0] == 'ROLE_TEACHER' ? "Вчитель" : editUser.roles[0] == 'ROLE_REVIEWER' ? 'Переглядач' : 'Учень'}</InputLabel>
                                        <Select fullWidth labelId="roles_label" name="roles" onChange={event => this.userChangeHandler(event)}
                                            inputProps={{
                                                readOnly: this.state.edit,
                                            }} label={editUser.roles[0] == 'ROLE_TEACHER' ? "Вчитель" : editUser.roles[0] == 'ROLE_REVIEWER' ? 'Переглядач' : 'Учень'}>

                                            <MenuItem value="ROLE_REVIEWER">
                                                Переглядач
                                            </MenuItem >
    ...
    userChangeHandler(event) {
        ...
        userCopy[event.target.name] = event.target.name === 'roles' ? [event.target.value] : event.target.value
        this.setState({
            editUser: userCopy
        })
    }
